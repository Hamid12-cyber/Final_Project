...
public static class ApproveMotorcycle
{
        ...
        public async Task Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            if (entity is null)
                throw new AppException("NOT_FOUND", StatusCodes.Status404NotFound,
                    $"Motorcycle (Id={request.Id}) tapılmadı.");
    ...
    {
        app.MapPut("/api/admin/motorcycles/{id:int}/approve", async (int id, ISender sender) =>
        {
            await sender.Send(new Command(id));
            return Results.NoContent();
        })
        .RequireAuthorization(policy => policy.RequireRole("Admin"))
        .WithName("ApproveMotorcycle")
        .WithTags("Admin");
    }
}